...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.CYAN}Cached Users: {Fore.GREEN}{len(xans.users)}
                       {Fore.CYAN}Guilds: {Fore.GREEN}{len(xans.guilds)}
                       {Fore.CYAN}Prefix: {Fore.GREEN}{xans.command_prefix}
    ''' + Fore.RESET)
...
xans.antiraid = False
xans.msgsniper = True
xans.slotbot_sniper = True
xans.giveaway_sniper = True
xans.mee6 = False
xans.mee6_channel = None
xans.yui_kiss_user = None
xans.yui_kiss_channel = None
xans.yui_hug_user = None
xans.yui_hug_channel = None
xans.sniped_message_dict = {}
xans.sniped_edited_message_dict = {}
xans.whitelisted_users = {}
xans.copycat = None
...
async def on_message_edit(before, after):
    await xans.process_commands(after)
        ...
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in xans.whitelisted_users.keys() and i.user.id in xans.whitelisted_users[
                    ...
                    await guild.ban(i.user, reason="xans Anti-Nuke")
...
@xans.command(aliases=["queue"])
async def play(ctx, *, query):
    await ctx.message.delete()
    voice = get(xans.voice_clients, guild=ctx.guild)
...
@xans.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    xans.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        xans.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        xans.slotbot_sniper = False
...
@xans.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    xans.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        xans.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        xans.giveaway_sniper = False
...
async def on_message_delete(message):
    ...
    if len(xans.sniped_message_dict) > 1000:
        xans.sniped_message_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
...
@xans.event
async def on_message_edit(before, after):
    if before.author.id == xans.user.id:
        return
    if xans.msgsniper:
    ...
    if len(xans.sniped_edited_message_dict) > 1000:
        xans.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
...
@xans.command(aliases=["addemoji", "stealemote", "addemote"])
...
@xans.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if xans.user is None:
        await ctx.send("You weren't copying anyone to begin with")
        return
    await ctx.send("Stopped copying " + str(xans.copycat))
    xans.copycat = None


@xans.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    xans.copycat = user
    await ctx.send("Now copying " + str(xans.copycat))


@xans.command(aliases=["9/11", "911", "terrorist"])
...
@xans.command(aliases=["jerkoff", "ejaculate", "orgasm"])
async def cum(ctx):
    await ctx.message.delete()
...
@xans.command(aliases=["spamchangegcname", "changegcname"])
...
@xans.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
    for guild in xans.guilds:
        await guild.ack()
...
@xans.command(aliases=["pornhublogo", "phlogo"])
...
@xans.command(aliases=["pornhubcomment", 'phc'])
...
@xans.command(aliases=['pfp', 'avatar'])
...
@xans.command(aliases=["del", "quickdel"])
...
@xans.command(name='1337speak', aliases=['leetspeak'])
...
@xans.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
                await xans.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@xans.command(name='set-pfp', aliases=['setpfp', 'pfpset,"changepfp'])
...
@xans.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@xans.command(aliases=['dong', 'penis'])
...
@xans.command(aliases=['changehypesquad'])
...
@xans.command(aliases=['tokenfucker', 'disable', 'crash'])
...
@xans.command(aliases=["copyguild", "copyserver"])
async def copy(ctx):  # b'\xfc'
    await ctx.message.delete()
    await xans.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in xans.guilds:
...
@xans.command(aliases=["rekt", "nuke"])
...
@xans.command(aliases=["kickall", "kickwave"])
...
@xans.command(aliases=["spamroles"])
...
@xans.command(aliases=["masschannels", "masschannel", "ctc"])
...
@xans.command(aliases=["delchannel"])
...
@xans.command(aliases=["deleteroles"])
...
@xans.command(aliases=["purgebans", "unbanall"])
...
async def dm(ctx, user: discord.Member, *, message):
    await ctx.message.delete()
    user = xans.get_user(user.id)
    if ctx.author.id == xans.user.id:
        return
    else:
        try:
...
@xans.command(aliases=['rainbowrole'])
...
@xans.command(aliases=["guildinfo"])
...
@xans.command(aliases=['slots', 'bet', "slotmachine"])
async def slot(ctx):
    await ctx.message.delete()
    emojis = "🍎🍊🍐🍋🍉🍇🍓🍒"
...
@xans.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
@xans.command(aliases=['serverbanner'])
...
@xans.command(aliases=["renameserver", "nameserver"])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    xans.command_prefix = str(prefix)
...
@xans.command(aliases=["changeregions", "changeregion", "regionschange"])
...
async def stream(ctx, *, message):
    ...
    await xans.change_presence(activity=stream)
...
@xans.command(aliases=["fliptable"])
